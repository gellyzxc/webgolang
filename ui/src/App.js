...
function App() {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:9090/api/notes", {
  ...
  const addNote = () => {
    axios
      .post(
        "http://localhost:9090/api/notes/add",
        {
      ...
      <div className="notes_display_block">
        <div className="header">
          ...
          <div className="header_buttons">
            <button className="add_btn" onClick={() => openAddModal()}>
              <img className="add_button" src={AddButton}></img>
            </button>
          </div>
        </div>
        {/* TILES START */}
            ...
            notes.map((note, index) => (
              <div className="note_block" key={index}>
                ...
                <div className={"buttons_container"}>
                  <button className="text_btn" onClick={() => DelNote(note.id)}>
                    <img className="add_button" src={DelButton}></img>
                  </button>
                  <button
                    className="text_btn_second"
                    onClick={() => openModalEdit(note.id)}
                  >
                    <img className="add_button" src={EditButton}></img>
                  </button>
                </div>
              </div>
            ))}
